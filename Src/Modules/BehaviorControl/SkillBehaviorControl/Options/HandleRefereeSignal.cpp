...
option((SkillBehaviorControl) HandleRefereeSignal,
       load((float)(2800.f) upperImageBorderAtHeight, /**< The height to look at (in mm). */
            (float)(2400.f) upperImageBorderAtHeightStandby, /**< The height to look at in standby (in mm). */
            (float)(515.f) assumedCameraHeight, /**< The assumed height of the camera above ground (in mm). */
            (Rangef)({1200.f, 9000.f}) distanceRange, /**< The distance range for this option to become active during kick-in. */
            (Rangea)({10_deg, 170_deg}) bearingRange, /**< The bearing range to the referee for this option to become active. */
            (int)(15000) kickInWaitTime, /**< How long to unsuccessfully look at referee during kick-in. */
            (Angle)(70_deg) maxHeadTurn, /**< Maximum head rotation before the body has to be turned. */
            (Angle)(2_deg) turnTolerance, /**< Accepted tolerance when reaching the required body rotation. */
            (Angle)(0_deg) player1StandbyHeadYawOffset,
            (Angle)(0_deg) player2StandbyHeadYawOffset,
            (Angle)(0_deg) player3StandbyHeadYawOffset,
            (Angle)(-5_deg) player4StandbyHeadYawOffset,
            (Angle)(0_deg) player5StandbyHeadYawOffset))
{
  ...
  state(lookAtReferee)
  {
    action
    {
      Angle standbyHeadYawOffset = 0_deg;
      if(theGameState.state == GameState::standby)
      {
        switch(theGameState.playerNumber)
        {
          case 1:
            standbyHeadYawOffset = player1StandbyHeadYawOffset;
            break;
          case 2:
            standbyHeadYawOffset = player2StandbyHeadYawOffset;
            break;
          case 3:
            standbyHeadYawOffset = player3StandbyHeadYawOffset;
            break;
          case 4:
            standbyHeadYawOffset = player4StandbyHeadYawOffset;
            break;
          case 5:
            standbyHeadYawOffset = player5StandbyHeadYawOffset;
            break;
        }
      }
      const Vector2f refereeLookOffset = theGameState.state == GameState::standby ? Pose2f(standbyHeadYawOffset) * refereeOffsetRelative : refereeOffsetRelative;

      LookAtPoint({.target = {refereeLookOffset.x(), refereeLookOffset.y(), lookAtHeight},
                   .camera = HeadMotionRequest::upperCamera});
